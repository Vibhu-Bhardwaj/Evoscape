...
const PreviewAd = ({ auth }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post(
        `https://evoscape-server-a8ey.onrender.com/previewad/${id}`,
        {},
        {
          headers: {
      ...
      setOwn(false);
      try {
        const notlogedindata = await axios.post(`https://evoscape-server-a8ey.onrender.com/previewad/notloggedin/${id}`);
  ...
  const handleRemove = async () => {
    try {
      setIsRemoving(true);
      await axios.delete(`https://evoscape-server-a8ey.onrender.com/myads_delete/${id}`, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
                  ...
                  <Stack spacing="1">
                    <Flex justify="space-between">
                      <Text color="blue.600" fontSize="xs">
                        {`${address.area ? `${address.area}, ` : ''}${address.city ? `${address.city}, ` : ''}${address.state ? `${address.state}` : ''}${address.postcode ? `, ${address.postcode}` : ''}`}
                      </Text>
