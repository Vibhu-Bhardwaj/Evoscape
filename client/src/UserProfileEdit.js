...
export default function UserProfileEdit() {
  ...
  useEffect(() => {
    const verifyEmail = async () => {
      try {
        const response = await fetch(`https://evoscape-server-a8ey.onrender.com/verification-status?email=${authemail}`);
  ...
  const handleSendOtp = async (event) => {
    setSending(true);
    try {
      const response = await axios.post("https://evoscape-server-a8ey.onrender.com/send-verification-email", { email });
  ...
  const handlePinSubmit = async (value) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://evoscape-server-a8ey.onrender.com/verify-email",
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      if (imageUrl.length > 200) {
        ...
        formData.append('upload_preset', 'evoscape');
  
        const { data } = await axios.post(
          'https://api.cloudinary.com/v1_1/dawozx6rn/image/upload',
          formData,
          {
            headers: {
      ...
      const response = await axios.post(
        'https://evoscape-server-a8ey.onrender.com/profile_edit',
